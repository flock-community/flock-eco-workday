...
export function DashboardHolidayChart() {
  ...
  useEffect(() => {
    const date = new Date();
    AggregationClient.holidayReportMe(date.getFullYear()).then((res) => setState(res));
  }, []);
  ...
  const data01 = [
    {name: 'Available', value: round(Math.max(total - state.holidayHours, 0))},
  ...
  return (
    <ResponsiveContainer height={400}>
      <PieChart >
        <Pie data={data01} dataKey="value" cx="50%" cy="50%" outerRadius={160} label={renderCustomizedLabel}>
          <Cell fill={"#d2d2d2"}/>
          <Cell fill={"#42a5f5"}/>
        </Pie>
        <Pie data={data02} dataKey="value" cx="50%" cy="50%" innerRadius={180} outerRadius={200} label>
          <Cell fill={"#6c6c6c"}/>
          <Cell fill={"#9e9e9e"}/>
          <Cell fill={"#ef5350"}/>
        </Pie>
