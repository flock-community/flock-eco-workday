...
export function EventRatingFeature({match}) {
  ...
  const renderRow = person => {
    const eventRating = ratings.find(rating => rating.person.code === person.code)
    return (
        ...
        {[...Array(10).keys()]
          ...
          .map(i => (
            <Grid key={`button-${person.code}-${i}`} item>
              <Fab
                color={eventRating && eventRating.rating === i ? "primary" : "default"}
  ...
  return (
    <Container maxWidth="md">
      ...
      {event.persons.map(person => (
        <Grid key={`person-${person.code}`} container spacing={3} alignItems="center">
          <Grid item xs>
            <Typography>
