...
describe("useExpenseFiltersHook", () => {
    ...
    const testExpense006 = createTestCostExpense('item-06', dayjs().subtract(30, 'days'), Status.DONE);
    const testExpense007 = createTestCostExpense('item-07', dayjs().subtract(31, 'days'), Status.REJECTED);
    ...
    describe("getOpenExpenses", () => {
        ...
        it('should return an empty list if the items provided do not match the filter criteria', () => {
            const result = getOpenExpenses([testExpense003, testExpense006, testExpense007]);
            expect(result.length).toBe(0);
        });
        ...
        it('should only return items with the status REQUESTED even when date is after numberOfDays(ForTest)', () => {
            ...
            expect(result[0].date.isBefore(dayjs().subtract(numberOfDaysForTest, 'days'))).toBeTruthy();
        });
    });
    ...
    describe("getRecentExpenses", () => {
        ...
        it('should return an empty list if the items provided do not match the status filter criteria', () => {
            const result = getRecentExpenses([testExpense001, testExpense002], numberOfDaysForTest);
            expect(result.length).toBe(0);
        });
        ...
        it('should only return items with the status REJECTED, APPROVED or DONE', () => {
            const result = getRecentExpenses([testExpense001, testExpense003, testExpense006,
                testExpense004], numberOfDaysForTest);
        ...
        it('should only return items with dates between today and the last 30 days', () => {
            ...
            const result = getRecentExpenses([testExpense003, testExpense004, testExpense005,
                testExpense006, testExpense007], lastNumberOfDays);
        ...
        it('should return item when date is equal to today', () => {
            ...
            const lastNumberOfDays = 25;
            const daysAgo = today.subtract(lastNumberOfDays, 'days');
            const result = getRecentExpenses([createTestTravelExpense('item-today', today,
                Status.APPROVED)], lastNumberOfDays);
            expect(result.length).toBe(1);
            expect(result[0].date.isBetween(today, daysAgo, 'days', []));
        });
        ...
        it('should not return item if the expense date is equal to numberOfDays + 1', () => {
            ...
            const result = getRecentExpenses([createTestTravelExpense('item-not-in-range',
                daysAgo.subtract(1, 'day'), Status.APPROVED)], lastNumberOfDays);
            expect(result.length).toBe(0);
        });
    });
    ...
    describe("split a list of expense objects", () => {
        it('should split all expenses from the list', () => {
            ...
            expect(expenses.length).toEqual(openExpensesResult.length + recentExpensesResult.length);
        });
    });
});
