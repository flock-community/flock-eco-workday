...
class GoogleExportController(
    ...
    fun exportWorkday(@PathVariable code: String, authentication: Authentication): ExportResponse {
        log.info("Exporting workday $code to google drive")
        val workday: WorkDay = service.findByCode(code)?.applyAuthentication(authentication)
