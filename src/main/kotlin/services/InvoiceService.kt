...
class InvoiceService(
                    ...
                    exactonlineDocumentClient.postDocument(requestObject, toExactonlineDocument())
                        .flatMapMany { exactDocument ->
                            Flux.fromIterable(documents)
                                .flatMap { document -> exactonlineDocumentClient.postDocumentAttachment(requestObject, document.toExactonlineDocumentAttachment(exactDocument)) }
                        }
                }
                .collectList()
                .doOnNext {
    ...
    private fun Document.toExactonlineDocumentAttachment(document: ExactonlineDocument) = ExactonlineDocumentAttachment(
