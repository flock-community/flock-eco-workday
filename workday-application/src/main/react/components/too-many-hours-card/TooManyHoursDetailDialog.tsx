...
import type { TooManyHoursPersonObject } from './TooManyHoursCard';

const PREFIX = 'TooManyHoursDetailDialog';
...
const classes = {
  ...
  dataItemExtra: `${PREFIX}dataItemExtra`,
};
...
const Root = styled('div')(() => ({
  ...
  [`& .${classes.dataItemWorkDay}`]: { backgroundColor: '#1de8b5' },
  [`& .${classes.dataItemHoliday}`]: { backgroundColor: '#42a5f5' },
  [`& .${classes.dataItemPaidPL}`]: { backgroundColor: '#ffb6c1' },
  [`& .${classes.dataItemUnpaidPL}`]: { backgroundColor: '#87cefa' },
  [`& .${classes.dataItemSickDay}`]: { backgroundColor: '#ef5350' },
  [`& .${classes.dataItemEventDay}`]: { backgroundColor: '#fed766' },
  [`& .${classes.dataItemExtra}`]: { backgroundColor: '#ff7043' },
}));

type TooManyHoursDetailDialogProps = {
  open: boolean;
  item: TooManyHoursPersonObject | undefined;
  onComplete: () => void;
};

export function TooManyHoursDetailDialog({
  open,
  item,
  onComplete,
}: TooManyHoursDetailDialogProps) {
    ...
    <Root>
      ...
      >
        <DialogHeader
          onClose={handleClose}
          icon={<Timeline />}
          headline={'Too many hours details'}
          ...
          {item && (
            <Box className={'flow'}>
              <div className={classes.flexDataContainer}>
                ...
                ></div>
                <div
                  className={classes.dataItemExtra}
                  style={{ flexGrow: state?.extra }}
                ></div>
              </div>
              ...
              <List dense={true}>
                ...
                <ListItem>
                  <ListItemIcon style={{ color: '#ff7043' }}>
                    <MoreHoriz />
                  </ListItemIcon>
                  <ListItemText primary={'Extra hours'} />
                  <ListItemSecondaryAction>
                    {state?.extra}
                  </ListItemSecondaryAction>
                </ListItem>
                ...
                <ListItem>
                  <ListItemIcon />
                  <ListItemText primary={'Expected total'} />
                  <ListItemSecondaryAction>
                    {state?.total}
                  </ListItemSecondaryAction>
                </ListItem>
