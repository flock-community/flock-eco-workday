...
- A new `KratosOpaqueTokenIntrospector` in `workday-application/.../config/` is registered via Spring Security's standard `oauth2ResourceServer().opaqueToken().introspector(...)`. The introspector calls Kratos `sessions/whoami` (with explicit connect/read timeouts so a slow Kratos cannot pin Tomcat threads, and a bounded LRU cache — positive ~60s, negative ~10s — to absorb token-spray without hammering Kratos), and resolves the Kratos identity to a workday `User` via `userService.findByEmail(traits.email)` — creating the `User` if missing. It then returns an `OAuth2AuthenticatedPrincipal` whose `name == user.code`, so downstream `authentication.name`-based lookups (e.g. `personService.findByUserCode`) keep working unchanged. Spring's stock `BearerTokenAuthenticationFilter` handles `Authorization: Bearer` parsing, 401 + `WWW-Authenticate` on invalid tokens, and `SecurityContext` population — the only project-specific code is the introspector itself. Note: Kratos `/sessions/whoami` is not RFC 7662, so this uses the `OpaqueTokenIntrospector` abstraction without the standard underneath it; the trade-off is deliberate (reviewer feedback: prefer Spring Security's library shape over a custom `GenericFilterBean`).
...
- The existing `WebSecurityConfig` filter chain remains; `UserKeyTokenFilter` keeps handling `Authorization: TOKEN <api-key>` and the OAuth2 resource-server filters handle `Authorization: Bearer <kratos-token>`. The two prefixes are disjoint.
- Operational simplification vs an earlier iteration: Kratos-unreachable failures surface as `OAuth2IntrospectionException`, which Spring renders as the standard 401 rather than the 503 the first filter-based draft returned. Mobile clients see a uniform 401 on any auth failure and re-auth; a custom `AuthenticationEntryPoint` to recover the 503 distinction was considered and rejected to keep the integration aligned with off-the-shelf Spring Security defaults.
- Side-effect on the existing API-key path: enabling `oauth2ResourceServer` switches Spring's exception handling from a single-entry-point setup to `DelegatingAuthenticationEntryPoint`, whose OAuth2 resource-server matcher is broad (Bearer header OR REST/non-HTML request). Unauthenticated REST API calls (e.g. an invalid `Authorization: TOKEN ...`) now return 401 instead of 302→`/`. This is the correct REST semantic — the previous 302 was an artifact of having only one entry point configured — but it is a visible behavior change for any client that relied on the redirect. `ApiKeyAuthenticationTest` is updated accordingly.
